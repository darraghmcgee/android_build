...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from Exynos5420 Github (http://github.com/exynos5420)." % device

repositories = []

page = 1
while not depsonly:
    request = Request("https://api.github.com/users/exynos5420/repos?page=%d" % page)
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/ld_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    f = open('.repo/local_manifests/ld_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/ld.dependencies'
    syncable_repos = []
...
print "Repository for %s not found in the exynos5420 Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/ld_manifest.xml" % device